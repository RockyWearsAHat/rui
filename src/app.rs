...
impl<S> App<S> {
    ...
    pub(crate) fn frame(
        ...
        self.frame_observed(canvas, fonts, input, memory, theme, &mut |_| {});
    }

    /// The same, handing every laid-out element to `observe` before the
...
/// Hands every element of a laid-out tree to `observe`, in the order they were
/// drawn.
fn visit<S>(el: &El<S>, observe: &mut dyn FnMut(&El<S>)) {
    observe(el);
