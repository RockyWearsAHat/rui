...
impl Canvas {
    ...
    /// Finds the smallest axis-aligned bounding rectangle that contains every
    /// pixel where `self` and `other` differ.
    ///
    /// Answers how little of the two canvases actually needs to be re-presented,
    /// making it safe to use a cheaper partial present on a region far smaller
    /// than the whole window — most commonly a button's hover highlight or a
    /// cursor animation. `None` if the canvases are identical everywhere.
    ///
    /// The caller must ensure both canvases have the same `width`, `height`, and
    /// `scale` — asserted in debug builds only so this is always fast enough to
    /// call every full frame as a presentation optimization. Unlike [`Self::crop`]
    /// and [`Self::snapshot`], this is a plain scan without copying pixel data,
    /// so the cost grows only with the window size, never with the region being
    /// tested.
    pub(crate) fn diff_bounds(&self, other: &Canvas) -> Option<Rect> {
        debug_assert_eq!(self.width, other.width);
        debug_assert_eq!(self.height, other.height);
        debug_assert_eq!(self.scale, other.scale);

        let pixels = self.pixels();
        let other_pixels = other.pixels();

        if pixels.len() != other_pixels.len() {
            return None;
        }
        ...
        let mut min_row: usize = height;
        let mut max_row: usize = 0;
        let mut min_col: usize = width;
        let mut max_col: usize = 0;

        let mut found_diff = false;

        // Scan row by row, using slice equality for quick rows.
        for y in 0..height {
            let row_start = y * width;
            let row_end = row_start + width;

            // Quick check: skip identical rows entirely.
            if pixels[row_start..row_end] != other_pixels[row_start..row_end] {
                found_diff = true;
                min_row = min_row.min(y);
                max_row = y;

                // Find column bounds only within rows that actually differ.
                for x in 0..width {
                    if pixels[row_start + x] != other_pixels[row_start + x] {
        ...
        // Convert device pixel bounds back to logical units, matching the
        // conversion in [`Self::crop`] and the pattern `device_bounds` uses.
        let scale = self.scale;
        let rect = Rect::new(
            (min_col as f32) / scale,
            (min_row as f32) / scale,
            ((max_col - min_col + 1) as f32) / scale,
            ((max_row - min_row + 1) as f32) / scale,
        );
    ...
    pub fn fill_radial(&mut self, center: Point, radius: f32, inner: Color, outer: Color) {
        ...
        let shade = Shade::Radial {
            inner,
            outer,
            cx,
            cy,
            inv_radius: 1.0 / r.max(SHADE_EPS),
        };
    ...
    pub fn blit_bgra(&mut self, dest: Rect, source: &Bgra<'_>) {
        ...
        let (Some(right), Some(bottom)) = (
            left.checked_add_unsigned(source.width()),
            top.checked_add_unsigned(source.height()),
        ) else {
            return;
        };
    ...
    fn new(center: (f32, f32), radius: f32, half: f32, start: f32, sweep: f32) -> Self {
        ...
        let ends = (sweep < TAU).then(|| {
            [
                ...
                on_circle(center, radius, start + sweep),
            ]
        });
        Self {
            center_x: center.0,
...
fn runs(spans: [Option<(f32, f32)>; 2], bounds: PixelBounds) -> impl Iterator<Item = (i32, i32)> {
...
mod tests {
    ...
    fn a_picture_is_clipped_by_the_canvas_edge_rather_than_wrapping() {
        ...
        assert_eq!(
            pixel_at(&canvas, 0, 3),
            Color::BLACK,
            "the picture wrapped onto the next row"
        );
        assert_eq!(
            pixel_at(&canvas, 1, 1),
            Color::BLACK,
            "nothing above or left of it"
        );
    }
    ...
    fn a_picture_at_a_negative_origin_shows_its_far_corner() {
        ...
        assert_eq!(
            pixel_at(&canvas, 0, 0).b,
            2 * 4 + 2,
            "the pixel at (2, 2) of the picture"
        );
    ...
    fn a_row_wider_than_the_picture_does_not_skew_it() {
        ...
        assert_eq!(
            stride, 32,
            "three pixels padded out to a multiple of sixteen"
        );
    ...
    fn a_picture_is_opaque_however_the_capture_left_its_alpha() {
        ...
        assert_eq!(
            canvas.pixels()[0] >> 24,
            0xff,
            "the surface stopped being opaque"
        );
    }
    ...
    fn a_picture_the_buffer_cannot_hold_is_refused_at_the_door() {
        ...
        assert_eq!(
            Bgra::new(4, 1, 8, &bytes),
            None,
            "a stride narrower than the row"
        );
        assert!(Bgra::new(2, 2, 8, &[0u8; 16]).is_some());
        assert!(
            Bgra::packed(0, 0, &[]).is_some(),
            "an empty picture is describable"
        );
    }
    ...
    fn a_picture_at_a_retina_scale_still_lands_pixel_for_pixel() {
        ...
        assert_eq!(
            pixel_at(&canvas, 2, 2),
            Color::WHITE,
            "logical (1, 1) is device (2, 2)"
        );
        assert_eq!(
            pixel_at(&canvas, 5, 5),
            Color::WHITE,
            "four device pixels across"
        );
        assert_eq!(pixel_at(&canvas, 1, 1), Color::BLACK);
    }
    ...
    fn a_ring_marks_its_band_and_neither_side_of_it() {
        ...
        for (x, y) in [(20u32, 8u32), (20, 32), (8, 20), (32, 20)] {
            assert!(
                pixel_at(&canvas, x, y).r > 200,
                "the band is missing at ({x}, {y})"
            );
        }
        assert_eq!(
            pixel_at(&canvas, 20, 20),
            Color::BLACK,
            "the middle is not filled"
        );
        assert_eq!(
            pixel_at(&canvas, 20, 1),
            Color::BLACK,
    ...
    fn an_arc_marks_only_what_it_sweeps() {
        ...
        assert!(
            pixel_at(&canvas, 32, 20).r > 200,
            "the sweep starts at the right"
        );
        assert!(pixel_at(&canvas, 20, 32).r > 200, "and ends at the bottom");
        assert_eq!(
            pixel_at(&canvas, 20, 8),
            Color::BLACK,
    ...
    fn a_ring_is_an_arc_that_goes_all_the_way_round() {
        ...
        assert!(
            pixel_at(&ring, 32, 20).r > 200,
            "a crack opened where the sweep closes"
        );
    }
    ...
    fn a_bands_halo_surrounds_it_on_both_sides_without_covering_it() {
        ...
        assert_eq!(
            pixel_at(&canvas, 20, 8),
            Color::BLACK,
            "the band is left for its caster"
        );
        assert!(
            pixel_at(&canvas, 20, 6).r > 0,
            "the halo should reach outward"
        );
        assert!(pixel_at(&canvas, 20, 12).r > 0, "and inward");
        assert_eq!(
            pixel_at(&canvas, 20, 0),
            Color::BLACK,
            "and no further than its blur"
        );
    }
    ...
    fn ticks_are_spaced_evenly_round_the_whole_circle() {
        ...
        assert_eq!(
            pixel_at(&canvas, 28, 28),
            Color::BLACK,
            "a tick appeared between two"
        );
        assert_eq!(
            pixel_at(&canvas, 20, 20),
            Color::BLACK,
            "the ticks reached the centre"
        );
    }
    ...
    fn a_line_marks_the_run_between_its_ends_and_stops_there() {
        ...
        assert!(
            pixel_at(&canvas, 10, 4).r > 200,
            "which is two pixels thick"
        );
        assert_eq!(pixel_at(&canvas, 10, 2), Color::BLACK, "and no thicker");
        assert_eq!(
            pixel_at(&canvas, 0, 5),
            Color::BLACK,
            "nothing before it begins"
        );
    ...
    fn a_diagonal_line_is_antialiased_along_its_whole_length() {
        ...
        assert!(
            partial > 10,
            "a diagonal drawn without antialiasing is a staircase"
        );
    }
    ...
    fn a_polyline_draws_every_pair_in_turn() {
        ...
        assert!(
            pixel_at(&canvas, 2, 2).r > 100,
            "and the corner they meet at"
        );
        assert_eq!(
            pixel_at(&canvas, 10, 10),
            Color::BLACK,
            "and nothing between the ends"
        );
    }
    ...
    fn a_ring_and_a_line_are_scaled_like_everything_else() {
        ...
        assert!(
            pixel_at(&canvas, 20, 8).r > 200,
            "logical 10 across and 4 up is device 20, 8"
        );
        assert_eq!(
            pixel_at(&canvas, 20, 20),
            Color::BLACK,
            "the hole is scaled too"
        );
        assert!(
            pixel_at(&canvas, 1, 20).r > 200,
            "a one-unit line is two pixels wide"
        );
    ...
    fn a_clip_confines_a_ring_and_a_line_as_it_does_a_rectangle() {
        ...
        assert_eq!(
            pixel_at(&canvas, 32, 20),
            Color::BLACK,
            "the band escaped its clip"
        );
        assert!(pixel_at(&canvas, 10, 4).r > 200);
        assert_eq!(
            pixel_at(&canvas, 30, 4),
            Color::BLACK,
            "the line escaped its clip"
        );
    }
    ...
    fn a_band_is_drawn_in_the_colour_it_was_given() {
        ...
        assert!(
            halo.b > halo.r,
            "the halo lost the hue it was cast in: {halo:?}"
        );
    }
    ...
    fn a_horizontal_gradient_runs_from_its_left_colour_to_its_right_one() {
        ...
        assert!(
            left.r < 8,
            "the left should be near the left colour, got {left:?}"
        );
        assert!(
            (120..=136).contains(&middle.r),
            "the middle should be halfway, got {middle:?}"
        ...
        assert!(
            right.r > 247,
            "the right should be near the right colour, got {right:?}"
        );
    }
    ...
    fn a_horizontal_gradient_column_is_one_even_colour_down_its_height() {
        ...
        for y in 0..40 {
            assert_eq!(
                pixel_at(&canvas, 30, y),
                expected,
                "the column is not even at y = {y}"
            );
        }
    }
    ...
    fn a_gradient_is_solid_beyond_either_end_of_its_axis() {
        ...
        assert_eq!(
            pixel_at(&canvas, 5, 5),
            Color::BLACK,
            "behind the axis stays c0"
        );
        assert_eq!(
            pixel_at(&canvas, 55, 5),
            Color::WHITE,
            "past the axis stays c1"
        );
    }
    ...
    fn a_radial_fill_is_brightest_at_its_centre() {
        ...
        assert!(
            center > 235,
            "the centre is near the inner colour, got {center}"
        );
        assert!(
            center > mid && mid > edge,
            "expected a falloff, got {center}, {mid}, {edge}"
        );
        assert_eq!(
            pixel_at(&canvas, 20, 2),
            Color::BLACK,
    ...
    fn a_radial_aura_fades_to_nothing_at_its_rim() {
        ...
        assert!(
            pixel_at(&canvas, 20, 20).r > 235,
            "near-solid at the centre"
        );
        let rim = pixel_at(&canvas, 20, 8).r;
        assert!(
            rim > 0 && rim < 255,
            "the aura is part-lit near its rim, got {rim}"
        );
    }
    ...
    fn a_beams_halo_reaches_beyond_its_core_and_stops_at_the_blur() {
        ...
        assert!(
            near > 0 && near < 255,
            "the halo is present but dimmer, got {near}"
        );
        assert!(near > far, "and fades outward, got {near} then {far}");
        assert_eq!(
            pixel_at(&canvas, 20, 30),
            Color::BLACK,
            "and stops at the blur"
        );
    }
