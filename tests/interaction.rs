...
fn a_press_that_wanders_off_the_control_does_not_activate_it() {
    ...
    harness
        .press(at)
        .drag_to(Point::new(at.x, at.y + 300.0))
        .release();
    assert_eq!(
        harness.state().count,
        0,
        "letting go elsewhere is how a click is cancelled"
    );
}
...
fn a_disabled_control_ignores_the_pointer_entirely() {
    ...
    let probe = harness
        .find("Start")
        .expect("it is still drawn, dimmed, rather than removed");
    assert!(
        probe.rect.w > 0.0,
        "a control that vanishes makes the row around it jump"
    );
}
...
fn only_the_topmost_thing_under_the_pointer_answers_it() {
    ...
    let mut harness = Harness::new(App::default(), |_: &App| {
        col(row((
            text("Row"),
            button("Open").on_click(|app: &mut App| app.count += 1),
        ))
        .h(40.0)
    ...
    assert_eq!(
        harness.state().count,
        1,
        "the row underneath must not fire as well"
    );
}
...
fn a_drag_reports_where_the_pointer_is_for_as_long_as_it_is_held() {
    ...
    assert!(
        (harness.state().volume - 0.25).abs() < 0.001,
        "the press itself sets a value"
    );
...
fn a_drag_ends_where_it_is_released_and_not_before() {
    ...
    let rect = harness
        .find_key("track")
        .expect("the track is on screen")
        .rect;
    harness
        .press(rect.center())
        .drag_to(Point::new(rect.x + 5.0, rect.y + 5.0));
    ...
    assert_eq!(
        harness.state().count,
        1,
        "exactly once, on the frame it was let go"
    );
}
...
fn a_physical_key_reaches_a_handler_even_where_the_library_has_no_name_for_it() {
    ...
    let mut harness = Harness::new(App::default(), |_: &App| {
        col(button("Viewport")
            .on_click(|_: &mut App| {})
            .on_raw_key(|app: &mut App, stroke| match stroke.phase {
    ...
    assert_eq!(
        harness.state().codes,
        [Some(function_key)],
        "a key with no name still arrived"
    );

    harness.raw_key_up(function_key, None);
    assert!(
        harness.state().codes.is_empty(),
        "the unnamed key was left held down"
    );
}
...
fn keys_reach_only_what_holds_the_keyboard() {
    let mut harness = Harness::new(App::default(), |_: &App| {
        col((
            button("First")
                .on_click(|_: &mut App| {})
                .on_key(|app: &mut App, key, _| app.keys.push(key)),
            button("Second")
                .on_click(|_: &mut App| {})
                .on_key(|app: &mut App, _, _| app.count += 1),
        ))
    });
    ...
    assert_eq!(
        harness.state().count,
        0,
        "the unfocused control heard nothing"
    );
}
...
fn the_keyboard_can_reach_and_activate_a_control_with_no_pointer_at_all() {
    ...
    assert_eq!(
        harness.state().count,
        1,
        "tab reaches the first control and Enter presses it"
    );
...
fn the_wheel_reaches_a_control_that_asked_for_it() {
    let mut harness = Harness::new(App::default(), |_: &App| {
        col(text("Zoom")
            .h(40.0)
            .on_scroll(|app: &mut App, _, down| app.wheel += down))
    });
...
fn the_wheel_goes_to_the_innermost_list_rather_than_the_page_around_it() {
    let mut harness = Harness::new(App::default(), |_: &App| {
        col(col((0..40)
            .map(|index| text(format!("row {index}")).h(20.0))
            .collect::<Vec<_>>())
        .key("inner")
        .h(100.0)
        .scroll())
    ...
    assert_eq!(
        harness.scroll_offset(inner.id),
        30.0,
        "the list under the pointer scrolled"
    );
    let outer = harness.find_key("outer").expect("the page is on screen");
    assert_eq!(
        harness.scroll_offset(outer.id),
        0.0,
        "and the page around it did not"
    );
}
...
fn typing_into_a_field_reports_the_text_it_now_holds() {
    ...
    assert_eq!(
        harness.state().typed,
        "mongo",
        "the caret is where the typing left it"
    );
}
...
fn an_unfocused_field_takes_no_typing() {
    ...
    assert_eq!(
        harness.state().typed,
        "",
        "typing goes where the keyboard is, and nowhere else"
    );
}
...
fn a_layer_is_drawn_over_what_it_was_opened_from_and_answers_the_pointer_first() {
    ...
    let mut harness = Harness::new(App::default(), |app: &App| {
        col((
            button("Sort")
                .on_click(|app: &mut App| app.menu_open = true)
                .add(app.menu_open.then(|| {
                    panel(col((
                        text("By name")
                            .h(24.0)
    ...
    assert!(
        !harness.shows("By name"),
        "a menu that is not open is not drawn"
    );
    ...
    assert!(
        menu.rect.intersect(under.rect).w > 0.0,
        "and it genuinely covers the row"
    );
    ...
    assert_eq!(
        harness.state().count,
        0,
        "the row underneath was covered, so it heard nothing"
    );
}
...
fn a_row_scrolled_out_of_its_list_cannot_be_clicked() {
    ...
    let first = harness
        .rect_of("row 0")
        .expect("the first row still has a position");
    assert!(
        first.max_y() <= list.rect.y,
        "it has been scrolled up out of its list"
    );
    assert!(
        first.y > 0.0,
        "and up into the heading, which is still inside the window"
    );
    ...
    assert_eq!(
        harness.state().count,
        0,
        "a row that scrolled away must not stay clickable"
    );
}
...
fn what_a_view_offers_can_be_asserted_without_drawing_anything_at_all() {
    ...
    assert!(
        !stopped.child(0).expect("Start").is_disabled(),
        "Start is offered when it is stopped"
    );
